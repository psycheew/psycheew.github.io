...
### 2. {% raw %}`{% comment %} ... {% endcomment %}`{% endraw %}
...
{% raw %}`{% comment %}`{% endraw %} 역시 Django에서 제공하는 템플릿 태그입니다.
...
Django 템플릿 코드 자체를 비활성화하고 싶다면 HTML 주석 대신 `{# ... #}` 또는 {% raw %}`{% comment %}`{% endraw %}를 사용하는 것이 안전합니다.


<br>


## 💡 {% raw %}`{% extends %}`{% endraw %}를 사용할 때는?
...
Django에서 템플릿 상속을 사용할 경우 {% raw %}`{% extends %}`{% endraw %}는 해당 템플릿의 **첫 번째 템플릿 태그**여야 합니다.


여기서 중요한 점은 단순히 파일의 물리적인 첫 줄이어야 한다는 의미와는 조금 다르다는 것입니다.


예를 들어 Django가 템플릿 태그로 취급하지 않는 내용과 {% raw %}`{% extends %}`{% endraw %}의 관계, 또는 사용 중인 Django 버전과 템플릿 구조에 따라 동작을 정확히 구분할 필요가 있습니다.


<br>


따라서 단순히 ‘{% raw %}`{% comment %}`{% endraw %}가 {% raw %}`{% extends %}`{% endraw %}보다 위에 있으면 무조건 오류가 발생한다.’라고 이해하기보다는, **{% raw %}`{% extends %}`{% endraw %}** **앞에 다른 템플릿 태그를 두지 않는 것**을 기본 원칙으로 기억하는 편이 정확합니다.
